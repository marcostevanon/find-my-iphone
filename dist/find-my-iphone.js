...
export class FindMyPhone {
    ...
    async login() {
        ...
        if (!res.data.webservices?.findme?.url)
    ...
    async getLocationOfDevice(device) {
        if (!device.location)
            throw new Error('No location in device');
        const { latitude, longitude } = device.location;
        const res = await axios.get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&sensor=true`);
        if (res.data.results?.[0]?.formatted_address) {
            return res.data.results[0].formatted_address;
        }
        throw new Error('No address found');
    }
    async getDistanceOfDevice(device, myLatitude, myLongitude) {
        if (!device.location)
            throw new Error('No location in device');
        const { latitude, longitude } = device.location;
        const res = await axios.get(`https://maps.googleapis.com/maps/api/distancematrix/json?origins=${myLatitude},${myLongitude}&destinations=${latitude},${longitude}&mode=driving&sensor=false`);
